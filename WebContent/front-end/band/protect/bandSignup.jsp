...
<body>
	<form method="post" enctype="multipart/form-data"
		action="<%=request.getContextPath()%>/band/band.do">
		<div class="col-12">
			<h1>
				樂團申請表單 <span class="badge badge-secondary"><i
			...
			<div class="row">
				<div class="col-5">
					...
					<c:if test="${memberVo.bandId!=null}">
						<div class="input-group" id="piececheck">
							<div class="custom-file">
								...
								<input type="file" class="custom-file-input" id="bandPieceCheck"
									name="bandPieceCheck"> <label class="custom-file-label"
									for="validatedInputGroupCustomFile">更換其他作品</label>
								<canvas id="canvas"></canvas>
								<audio id="audio" controls>
									<source
										src="<%=request.getContextPath() %>/band/band.do?action=getSong&bandId=${memberVo.bandId}"
										type="audio/ogg">
								</audio>
							</div>
					...
					<c:if test="${memberVo.bandId==null}">
						<div class="input-group" id="piececheck">
							<div class="custom-file">
								...
								<input type="file" class="custom-file-input" id="bandPieceCheck"
									name="bandPieceCheck" required> <label
									class="custom-file-label" for="validatedInputGroupCustomFile">請上傳一首作品</label>
								<canvas id="canvas"></canvas>
								<audio id="audio" controls>
								</audio>
							</div>
					...
					<div class="mb-3">
						...
						<c:if test="${memberVo.bandId==null}">
							<textarea class="form-control" id="bandIntro" name="bandIntro"
								placeholder="請輸入樂團介紹" required></textarea>
						</c:if>
						<c:if test="${memberVo.bandId!=null}">
							<textarea class="form-control" id="bandIntro" name="bandIntro"
								required>${bandVo.band_intro}</textarea>
						</c:if>
					...
					<c:if test="${memberVo.bandId==null}">
						<div class="input-group" id="bandbannerpic">
							<div class="custom-file">
								...
								<input type="file" class="custom-file-input" id="bandBanner"
									name="bandBanner" required> <label
									class="custom-file-label" for="validatedInputGroupCustomFile">請上傳一張樂團封面照</label>
							</div>

						</div>
						<div class="invalid-feedback">必須上傳一張封面照片</div>
						<div class="col-7" id="bannerpic">
							<img src="<%=request.getContextPath()%>/images/無圖片.png"
					...
					<c:if test="${memberVo.bandId!=null}">
						<div class="input-group" id="bandbannerpic">
							<div class="custom-file">
								...
								<input type="file" class="custom-file-input" id="bandBanner"
									name="bandBanner"> <label class="custom-file-label"
									for="validatedInputGroupCustomFile">請上傳一張樂團封面照</label>
							</div>
						...
						<div class="col-7" id="bannerpic">
							<img
								src="<%=request.getContextPath()%>/band/band.do?action=getBanner&bandId=${memberVo.bandId}"
								class="img-fluid" alt="Responsive image">
						</div>
					</c:if>
				</div>
