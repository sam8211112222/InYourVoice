			...
			<div class="row">
				<div class="col-5">
					<div class="form-group">
						...
						<input type="text"
							class="form-control" id="bandName" name="bandName" value="${bandVo.band_name}">
						<small id="emailHelp" class="form-text text-muted">請輸入完整名稱</small>
						</c:if>
						<c:if test="${memberVo.bandId==null}">
						<input type="text"
							class="form-control" id="bandName" name="bandName" required>
						<small id="emailHelp" class="form-text text-muted">請輸入完整名稱</small>
						</c:if>
					</div>
					<c:if test="${memberVo.bandId!=null}">
					<div class="input-group" id="piececheck">
						<div class="custom-file">
							...
							<input type="file" class="custom-file-input" id="bandPieceCheck"
								name="bandPieceCheck"> <label
								class="custom-file-label" for="validatedInputGroupCustomFile">更換其他作品</label>
				...
				<div class="col-7">
					...
					<div class="invalid-feedback">必須上傳一張封面照片</div>
					<div class="col-7" id="bannerpic">
						<img src="<%=request.getContextPath()%>/band/band.do?action=getBanner&bandId=${memberVo.bandId}"
							class="img-fluid" alt="Responsive image">
					</div>
					</c:if>
				</div>
