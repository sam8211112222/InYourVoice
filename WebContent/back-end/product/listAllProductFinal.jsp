...
<%
    ...
    List<ProductVO> list = ProductSvc.getTime();
    pageContext.setAttribute("list",list);
%>

<html>
<head>
<title>修改後所有商品資料 - listAllProductFinal.jsp</title>
<style>
  table#table-1 {
...
<table id="table-1">
	<tr><td>
		 <h3>修改後所有商品資料 - listAllProductFinal.jsp</h3>
...
<div><font color="red">已修改完成</font></div>
		...
		<tr>
			...
			<c:choose>
				...
				<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/product/product.do" style="margin-bottom: 0px;">
			     <input type="submit" value="點此確認審核" id="submit" >
			...
			<c:when test="${productVO.product_status == 1}">
			  <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/product/product.do" style="margin-bottom: 0px;">
			     已上架
			     <input type="submit" value="按此下架" id="submit">
			     <input type="hidden" name="product_id"  value="${productVO.product_id}">
			...
			<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/product/product.do" style="margin-bottom: 0px;">
			      已下架
			     <input type="submit" value="按此上架" id="submit">
			     <input type="hidden" name="product_id"  value="${productVO.product_id}">
